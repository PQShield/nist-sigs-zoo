...
test.describe('KEMs page', () => {
	...
	test('table lists all KEM parameter sets', async ({ page }) => {
		...
		for (const name of ['ML-KEM-768', 'HQC-256', 'FrodoKEM-640-AES', 'mceliece6960119', 'BAT-769-1024', 'X25519', 'P-256']) {
	...
	test('level filter excludes pre-quantum schemes', async ({ page }) => {
		...
		await expect(page.getByText('19 parameter sets')).toBeVisible();
