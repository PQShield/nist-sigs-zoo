...
test.describe('KEMs page', () => {
	...
	test('table lists all KEM parameter sets', async ({ page }) => {
		...
		await expect(page.getByText('27 parameter sets')).toBeVisible();
		for (const name of ['ML-KEM-768', 'HQC-256', 'FrodoKEM-640-AES', 'mceliece6960119', 'BAT-769-1024', 'ntruhps2048509', 'X25519', 'P-256']) {
	...
	test('level filter excludes pre-quantum schemes', async ({ page }) => {
		...
		await expect(page.getByText('23 parameter sets')).toBeVisible();
