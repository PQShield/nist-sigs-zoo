 ...
 *
 * All return 0 on success.  This is the standard NIST KEM API.
 *
 * The .so is opened with RTLD_LOCAL so non-namespaced implementations
 * (e.g. bare `crypto_kem_keypair` from several upstreams at once) cannot
...
typedef struct {
    ...
    int (*encaps_fn)(uint8_t *ct, uint8_t *ss, const uint8_t *pk);
    int (*decaps_fn)(uint8_t *ss, const uint8_t *ct, const uint8_t *sk);
    /* internal */
    void *dl_handle;
} bench_scheme_t;
