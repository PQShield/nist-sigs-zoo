#!/usr/bin/env bash
# Fetch + build the libmceliece static library and its randombytes/cpucycles
# dependencies into a local prefix, then harvest the .a archives the shims link
# against. Idempotent: re-running is a no-op once build/local/.done exists.
#
...
CC_VER=20260105;  CC_URL="https://cpucycles.cr.yp.to/libcpucycles-${CC_VER}.tar.gz"
CC_SHA=e87dcaaa28e905b574ccf3d49e23e05c73edb3f99136dcd566bca16829ab6694
...
fetch "$CC_URL" "$CC_SHA" cc.tar.gz
fetch "$MC_URL" "$MC_SHA" mc.tar.gz

rm -rf "$SRC"/librandombytes-* "$SRC"/libcpucycles-* "$SRC"/libmceliece-*
tar xzf "$DL/rb.tar.gz" -C "$SRC"
tar xzf "$DL/cc.tar.gz" -C "$SRC"
tar xzf "$DL/mc.tar.gz" -C "$SRC"

# librandombytes + libcpucycles: tiny djb libs, install into the local prefix.
echo "  build librandombytes"
( cd "$SRC/librandombytes-$RB_VER" && ./configure --prefix="$LOCAL" >/dev/null && make -j"$JOBS" install >/dev/null )
echo "  build libcpucycles"
( cd "$SRC/libcpucycles-$CC_VER" && ./configure --prefix="$LOCAL" >/dev/null && make -j"$JOBS" install >/dev/null )
...
# We harvest the static archive directly instead of `make install` (which would
# also build the CLI/man pages and write outside the prefix).
...
( cd "$SRC/libmceliece-$MC_VER" \
    ...
    && CPATH="$LOCAL/include" LIBRARY_PATH="$LOCAL/lib" make -j"$JOBS" >/dev/null 2>&1 || true )
