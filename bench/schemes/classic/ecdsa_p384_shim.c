...
static const bench_scheme_info_t INFO = { "ECDSA-P384 (SHA-384)", 97, 167, 104, 0 };
const bench_scheme_info_t *bench_info(void) { return &INFO; }

int crypto_sign_keypair(uint8_t *pk, uint8_t *sk) {
    EVP_PKEY_free(g_key);
    g_key = EVP_EC_gen("P-384");
    (void)pk; (void)sk;
    return g_key ? 0 : -1;
}
...
int crypto_sign_signature(uint8_t *sig, size_t *siglen,
    ...
    if (!g_key) return -1;
    EVP_MD_CTX *ctx = sign_ctx_new(g_key, EVP_sha384(), NULL);
    if (!ctx) return -1;
    *siglen = 104;
...
int crypto_sign_verify(const uint8_t *sig, size_t siglen,
    ...
    if (!g_key) return -1;
    EVP_MD_CTX *ctx = vrfy_ctx_new(g_key, EVP_sha384(), NULL);
    if (!ctx) return -1;
