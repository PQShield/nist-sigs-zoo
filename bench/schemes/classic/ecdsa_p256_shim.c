...
/* ECDSA sig is DER-encoded; max for P-256 is 72 bytes. */
static const bench_scheme_info_t INFO = { "ECDSA-P256 (SHA-256)", 65, 121, 72, 0 };
...
int crypto_sign_signature(uint8_t *sig, size_t *siglen,
    ...
    if (!g_key) return -1;
    EVP_MD_CTX *ctx = sign_ctx_new(g_key, EVP_sha256(), NULL);
    if (!ctx) return -1;
    *siglen = 72;
...
int crypto_sign_verify(const uint8_t *sig, size_t siglen,
    ...
    if (!g_key) return -1;
    EVP_MD_CTX *ctx = vrfy_ctx_new(g_key, EVP_sha256(), NULL);
    if (!ctx) return -1;
