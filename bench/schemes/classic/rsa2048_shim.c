...
static const bench_scheme_info_t INFO = { "RSA-2048 (PSS)", 294, 1219, 256, 200 };
...
int crypto_sign_signature(uint8_t *sig, size_t *siglen,
    ...
    EVP_MD_CTX *ctx = sign_ctx_new(g_key, EVP_sha256(), &pctx);
    if (!ctx) return -1;
    int ret = -1;
    *siglen = 256;
...
int crypto_sign_verify(const uint8_t *sig, size_t siglen,
    ...
    if (!g_key) return -1;
    EVP_PKEY_CTX *pctx = NULL;
    EVP_MD_CTX *ctx = vrfy_ctx_new(g_key, EVP_sha256(), &pctx);
    if (!ctx) return -1;
    int ret = -1;
    if (EVP_PKEY_CTX_set_rsa_padding(pctx, RSA_PKCS1_PSS_PADDING) > 0 &&
