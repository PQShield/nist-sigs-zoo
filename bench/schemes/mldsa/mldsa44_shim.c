/*
 * Shim for ML-DSA-44 (dilithium mode 2, pq-crystals/dilithium reference).
...
int crypto_sign_signature(uint8_t *sig, size_t *siglen,
                          const uint8_t *m, size_t mlen,
                          const uint8_t *sk) {
    return pqcrystals_dilithium2_ref_signature(sig, siglen, m, mlen, NULL, 0, sk);
}

int crypto_sign_verify(const uint8_t *sig, size_t siglen,
                       const uint8_t *m, size_t mlen,
                       const uint8_t *pk) {
    return pqcrystals_dilithium2_ref_verify(sig, siglen, m, mlen, NULL, 0, pk);
}
